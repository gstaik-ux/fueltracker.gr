import { notFound } from "next/navigation";
import { query } from "@/lib/db";
import VehicleDashboard from "./VehicleDashboard";

export const dynamic = "force-dynamic";

export default async function VehiclePage({ params }: { params: { slug: string } }) {
  const vehicleResult = await query(
    "select id, name, theme_accent, theme_bg, vehicle_icon, tank_capacity from vehicles where slug = $1",
    [params.slug]
  );
  const vehicle = vehicleResult.rows[0];
  if (!vehicle) notFound();

  const fillupsResult = await query(
    `select id, date, liters, cost, odometer, odometer_estimated, is_trip, is_full
     from fillups where vehicle_id = $1 order by date desc, created_at desc`,
    [vehicle.id]
  );
  const serviceResult = await query(
    `select id, type, date, odometer, note
     from service_entries where vehicle_id = $1 order by date desc, created_at desc`,
    [vehicle.id]
  );

  return (
    <VehicleDashboard
      slug={params.slug}
      vehicleName={vehicle.name}
      themeAccent={vehicle.theme_accent}
      themeBg={vehicle.theme_bg}
      vehicleIcon={vehicle.vehicle_icon}
      tankCapacity={vehicle.tank_capacity != null ? Number(vehicle.tank_capacity) : null}
      initialFillups={fillupsResult.rows.map((r: any) => ({
        id: r.id,
        date: typeof r.date === "string" ? r.date : r.date.toISOString().slice(0, 10),
        liters: Number(r.liters),
        cost: Number(r.cost),
        odometer: r.odometer == null ? null : Number(r.odometer),
        odometerEstimated: !!r.odometer_estimated,
        isTrip: !!r.is_trip,
        isFull: !!r.is_full,
      }))}
      initialServiceEntries={serviceResult.rows.map((r: any) => ({
        id: r.id,
        type: r.type,
        date: typeof r.date === "string" ? r.date : r.date.toISOString().slice(0, 10),
        odometer: r.odometer == null ? null : Number(r.odometer),
        note: r.note || "",
      }))}
    />
  );
}
